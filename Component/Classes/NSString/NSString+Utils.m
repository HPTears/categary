...
/**
 对url 中的 中文字符转为utf-8（不是二次转码）
 */
- (NSString *)encodeURLIfContainZN_CH  {
...
/**
 查找searchString在字符串中的位置，所有位置
 NSArray 里是NSRange
 */
- (NSArray <NSValue *> *)rangesOfString:(NSString *)searchString {
    NSString *tempString = self.copy;
    NSRange range = [tempString rangeOfString:searchString options:NSBackwardsSearch range:NSMakeRange(0, tempString.length)];
    //循环检索
    NSMutableArray *rangesArray = @[].mutableCopy;
    while(range.location != NSNotFound) {
        [rangesArray addObject:[NSValue valueWithRange:range]];
        NSUInteger start = 0;
        NSUInteger end = range.location;
        NSRange temp = NSMakeRange(start,end);
        range =[tempString rangeOfString:searchString options:NSBackwardsSearch range:temp];
    }
